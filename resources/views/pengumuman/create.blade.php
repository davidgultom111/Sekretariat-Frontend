@extends('layouts.app')
@section('title', 'Tambah Pengumuman — Sekretariat GPdI')
        ...
        <form action="{{ route('pengumuman.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="judul" value="{{ old('judul') }}"
                       ...
                       placeholder="cth: Ibadah Natal 25 Desember 2026" required>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Mulai Tayang <span class="text-red-500">*</span></label>
                    <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', date('Y-m-d')) }}"
                           class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('tanggal_mulai') border-red-400 @enderror"
                           required>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_akhir" value="{{ old('tanggal_akhir') }}"
                           class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('tanggal_akhir') border-red-400 @enderror">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors">Simpan Pengumuman</button>
            </div>
        </form>
    </div>
</div>
