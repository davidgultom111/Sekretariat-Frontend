@extends('layouts.app')
@section('title', 'Jadwal Pelayanan — Sekretariat GPdI')
@section('page-title', 'Jadwal Pelayanan')
...
<div x-data="{ show: false, deleteId: null, deleteName: '' }">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-gray-800">Jadwal Pelayanan</h1>
            <p class="text-sm text-gray-500 mt-0.5">Kelola jadwal kegiatan gereja yang tampil di website</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-100">
                <tr>
                    ...
                    <th class="text-left px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Kegiatan</th>
                    <th class="text-left px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kategori</th>
                    <th class="text-left px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Hari & Waktu</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-medium text-gray-800">{{ $j['nama_kegiatan'] }}</div>
                        @if($j['deskripsi'])
                            <div class="text-xs text-gray-400 mt-0.5 line-clamp-1">{{ $j['deskripsi'] }}</div>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-block px-2.5 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full">
                            {{ $j['kategori'] }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <button @click="show=true; deleteId={{ $j['id'] }}; deleteName='{{ $j['nama_kegiatan'] }}'"
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-gray-400 text-sm">Belum ada jadwal. Klik "Tambah Jadwal" untuk memulai.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-800">Hapus Jadwal</h3>
                    <p class="text-sm text-gray-500">Tindakan ini tidak dapat dibatalkan</p>
                </div>
            </div>
            <p class="text-sm text-gray-600 mb-5">Yakin ingin menghapus jadwal <strong x-text="deleteName"></strong>?</p>
            <div class="flex gap-3">
                ...
                <form :action="'/jadwal/' + deleteId" method="POST" class="flex-1">
                    @csrf @method('DELETE')
